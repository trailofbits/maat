...
RUN git clone -b 0.11.5 --depth=1 https://github.com/lief-project/LIEF && \
  cmake -B LIEF/build -S LIEF -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_INSTALL_PREFIX=/opt/LIEF && \
...
RUN git clone -b z3-4.8.14 --depth=1 https://github.com/Z3Prover/z3 && \
  cmake -B z3/build -S z3 -DZ3_BUILD_LIBZ3_SHARED=OFF -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_INSTALL_PREFIX=/opt/z3 && \
  cmake --build z3/build -j 4 && \
  cmake --install z3/build && \
  rm -rf z3

# Install Sleigh (need git stuff for patching...)
RUN git clone -b v10.1.2 --depth=1 https://github.com/lifting-bits/sleigh && \
  git config --global user.email root@buildkitsandbox.fake && \
  git config --global user.user root && \
  cmake -B sleigh/build -S sleigh -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_INSTALL_PREFIX=/opt/sleigh && \
...
RUN cmake -S . -B /tmp/maat/build -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_INSTALL_PREFIX=/opt/maat \
    ...
    && \
  cmake --build /tmp/maat/build -j 4 && \
